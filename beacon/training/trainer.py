...
def _collect_guide_losses(model, guide_loader, device, dtype):
    model.eval()
    losses = []
    with torch.no_grad():
...
def _evaluate(model, loader, device, split_name="eval"):
    model.eval()
...
def train(cfg, model, tokenizer, ds_tokenized, train_embs, guide_embs, device, dtype):
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        if epoch > 0:
            ...
            print(f"  Weights — std:{w_stats['w_std']:.4f} max:{w_stats['w_max']:.4f} ratio:{w_stats['w_ratio']:.2f}x")
    ...
    best_model = AutoModelForSequenceClassification.from_pretrained(output_dir, torch_dtype=dtype).to(device)
